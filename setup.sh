...
read -p "Enter the subdomain prefix (e.g., chat, matrix): " PREFIX
read -p "Enter your domain name (e.g., mydomain.net): " DOMAIN
...
if [[ -n "$PREFIX" ]]; then
    # If the prefix is provided, combine it with the domain
    NEW_CONFIG_STRING="${PREFIX}.${DOMAIN}"
else
    echo "Error: Subdomain name cannot be empty."
    exit 1
fi
...
if [[ ! -f "$CADDYFILE" ]]; then
        echo "Warning: Caddyfile not found. Generating"
        cat << EOF > "$CADDYFILE"
...
EOF
    ...
    OLD_CONFIG_STRING="matrix.your-domain.com"
    OLD_DOMAIN="your-domain.com"
    sed -i "s/$OLD_CONFIG_STRING/$NEW_CONFIG_STRING/g" "$CADDYFILE"
    sed -i "s/$OLD_DOMAIN/$DOMAIN/g" "$CADDYFILE"
fi

mkdir -p "ddns_data"
DYNDNSFILE='ddns_data/config.json'

if [[ ! -f "$DYNDNSFILE" ]]; then
        echo "Warning: Dynamic DNS config not found. Generating"
        cat << EOF > "$DYNDNSFILE"
{
	"settings": [
		{
...
DBPASS=$(generate_random_string 16)
echo "Done"
...
DOCKERFILE="docker-compose.yml"
        cat << EOF > "$DOCKERFILE"
services:
  # 1. The Matrix Server (Continuwuity) and database
  continuwuity:
    ...
    ports:
      - "6167:6167"  # remove 127.0.0.1 if you need to bypass the reverse proxy externally.
    volumes:
...
echo "Automatic setup done."
echo "What you still need to do:"
echo "1. Configure ddns_data/config.json"
echo "    The configuration depends on your provider and you need to provide "
echo "    some kind of private key. More information: "
echo "    https://github.com/qdm12/ddns-updater/tree/master/docs"
echo "2. Add users to your server. You can call the add_user.sh utility to "
echo "    facilitate the process. It is recommended to create at least one admin"
echo "    user account. The utility will guide you."
